...
export function createRippleField(width: number, height: number, minimumCellSize = RIPPLE_CELL_SIZE): RippleField {
  const cellSize = Math.max(minimumCellSize, Math.sqrt(width * height / RIPPLE_MAX_CELLS));
